...
export function createWorldProxy<T extends readonly ContractDefinition[]>(
    ...
    for(let contractDef of contractDefinitions) {
        ...
        const functionAbis = contract.abi.filter(
            (item): item is FunctionAbi => item.type === "function"
        ) as unknown as FunctionAbis;

        for(const functionAbi of (functionAbis as readonly FunctionAbi[])) {
        ...
        proxy[contract.tag as keyof WorldContracts<T>] = functions;
    }
