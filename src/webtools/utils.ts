...
export const isCoordinateInBounds = (
    [x, y]: Coordinate,
    { upperLeft, upperRight, lowerLeft }: BoardBounds,
) => {
    ...
    if (upperRight[0] > upperLeft[0]) {
        ...
        xIsInBounds =
            (x >= upperLeft[0] && x > upperRight[0]) || (x < upperLeft[0] && x <= upperRight[0]);
    }
    ...
    if (upperRight[1] < lowerLeft[1]) {
        ...
        yIsInBounds =
            (y >= upperRight[1] && y > lowerLeft[1]) || (y < upperRight[1] && y <= lowerLeft[1]);
    }

    return xIsInBounds && yIsInBounds;
};

export const createBoardBounds = (
...
export function getWrappedTileCoordinate(
    startingWorldCoordinate: number,
    index: number,
...
export function handlePixelChanges(
    ...
    const [newPixelOffsetX, newWorldOffsetX] = handlePixelChange(
        pixelOffset[0],
        worldOffset[0],
        change[0],
        cellWidth,
    );
    const [newPixelOffsetY, newWorldOffsetY] = handlePixelChange(
        pixelOffset[1],
        worldOffset[1],
        change[1],
        cellWidth,
...
export function updateWorldTranslation(
    [worldX, worldY]: Coordinate,
    [cellX, cellY]: Coordinate,
): Coordinate {
...
if (import.meta.vitest) {
    ...
    describe('worldToView should correctly return view coordinates', () => {
        it('all 0, should return all 0 too', () => expect(worldToView(0, 0, 10, 100)).toEqual(0));

        it('shifting 2 right from origin', () => expect(worldToView(2, 0, 10, 100)).toEqual(20));

        it('shifting 1 right from origin', () => expect(worldToView(1, 0, 10, 100)).toEqual(10));

        it('shifting 2 left from origin', () => expect(worldToView(-2, 0, 10, 100)).toEqual(-20));

        it('shifting 1 left from origin', () => expect(worldToView(-1, 0, 10, 100)).toEqual(-10));

        it('crossing boundary: all 0, should return all 0 too', () =>
            expect(worldToView(1, 4294967200, 10, 100)).toEqual(-940));

        it('not crossing boundary: 105 left of 4294967100', () =>
            // raw: 4294967205
            //
            expect(worldToView(105, 4294967100, 10, 100)).toEqual(-950));
    });

    describe('viewToWorld should correctly return world coordinates', () => {
        it('all 0, should return all 0 too', () =>
            expect(applyWorldOffset([0, 0], [0, 0])).toEqual([0, 0]));

        it('No translation should result in same as input', () =>
            expect(applyWorldOffset([0, 0], [1, 1])).toEqual([1, 1]));

        it('Translation by negative 1 of 0 should give -1 (max_uint)', () =>
            expect(applyWorldOffset([1, 1], [0, 0])).toEqual([4_294_967_295, 4_294_967_295]));

        it('Moving [-1,-1] by 2 to rightbottom', () =>
            expect(
                applyWorldOffset([4_294_967_294, 4_294_967_294], [4_294_967_295, 4_294_967_295]),
            ).toEqual([1, 1]));
    ...
    describe('updateWorldTranslation should correctly update world coordinates', () => {
        ...
        it('should handle negative translation correctly', () =>
            expect(updateWorldTranslation([-1, -1], [-1, -1])).toEqual([
                MAX_UINT32 - 1,
                MAX_UINT32 - 1,
            ]));
        ...
        it('should wrap around correctly', () =>
            expect(updateWorldTranslation([MAX_UINT32 - 1, MAX_UINT32 - 1], [2, 2])).toEqual([
                0, 0,
            ]));
    ...
    describe('applyWorldOffset should return correct cell coordinates', () => {
        it('zeroes', () => expect(applyWorldOffset([0, 0], [0, 0])).toEqual([0, 0]));

        it('0 minus one', () => expect(applyWorldOffset([4294967295, 0], [0, 0])).toEqual([1, 0]));

        it('160 minus 1', () =>
            expect(applyWorldOffset([4294967295, 0], [160, 0])).toEqual([161, 0]));

        it('-1 plus 1', () => expect(applyWorldOffset([-1, 0], [-1, 0])).toEqual([0, 0]));

        it('plus 1', () => expect(applyWorldOffset([1, 0], [0, 0])).toEqual([4294967295, 0]));
        it('plus 95', () => expect(applyWorldOffset([95, 0], [0, 0])).toEqual([4294967201, 0]));
    });

    describe('uint2relative', () => {
        it('1', () => {
            expect(uint2relative(4294967200)).toEqual(-96);
            expect(uint2relative(4294967295)).toEqual(-1);
            expect(uint2relative(500)).toEqual(500);
            expect(uint2relative(0)).toEqual(0);
        });
    });
    ...
    describe('cellForPosition should return correct cell coordinates', () => {
        it('should return [0, 0] for 1 pixel onscreen is 1 in the world, and pos is topleft', () =>
            expect(cellForPosition(1000, [0, 0], [0, 0])).toEqual([0, 0]));
        ...
        it('should handle offset correctly for 1 pixel onscreen is 10 in the world, and pos is topleft with another specific offset', () =>
            expect(cellForPosition(1000, [1, 1], [4, 4])).toEqual([1, 1]));

        it('should handle larger world coordinates correctly', () =>
            expect(cellForPosition(1000, [98, 98], [200, 200])).toEqual([11, 11]));

        it('realistic', () =>
            expect(cellForPosition(10000, [0, 25], [298.5, 44.15625])).toEqual([1, 0]));
    });
