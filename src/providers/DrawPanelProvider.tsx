...
type DrawPanelType = {
  ...
  onVisibleAreaCoordinate?: (
    visibleAreaStart: Coordinate,
    visibleAreaEnd: Coordinate
  ) => void;
  onHover?: (coordinate: Coordinate) => void;
};

export const DrawPanelContext = React.createContext<DrawPanelType>(
  {} as DrawPanelType
);

export default function DrawPanelProvider({
  children,
}: {
  ...
  const { zoomLevel, notificationData } = useGameStore((state) => {
    return {
  ...
  const [visibleAreaStart, setVisibleAreaStart] = React.useState<
    [number, number]
  >([0, 0]);
  const [visibleAreaEnd, setVisibleAreaEnd] = React.useState<[number, number]>([
    28, 8,
  ]);
  ...
  const handleCellClick = (coordinate: Coordinate) => {
    const pixel = pixels.find(
      (pixel) => pixel!.x === coordinate[0] && pixel!.y == coordinate[1]
    );
    getGameStore().set({
  ...
  const handleVisibleAreaCoordinate = (
    visibleAreaStart: Coordinate,
    visibleAreaEnd: Coordinate
  ) => {
