...
const Alert: React.FC<AlertProp> = ({position, caller, message, className, onAlertClick, id, read}) => {
    ...
    return (
        ...
        >
            <div onClick={handleOnClickNotification}
                 className={'cursor-pointer w-[95%] pr-[5px]'}>{name} - {message}</div>
            {read !== true && (
                ...
                >
                    ...
                    >
                        <div className={cn(['grow py-xs'])}>
                            <h2 className={cn(['text-[#FFC400] text-left text-base uppercase font-silkscreen'])}>Event
                                Logs</h2>
                        </div>
                        <Button
                            variant={'icon'}
