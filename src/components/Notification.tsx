  ...
  return (
    ...
    >
      <div
        ...
        className={"cursor-pointer w-[95%] pr-[5px]"}
      >
        {name} - {message}
      </div>
      {read !== true && (
...
export default function Notification() {
  ...
  const [readAlerts, setReadAlerts] = useLocalStorage<string[]>(
    `pixelaw::read_alerts::${accountAddress}`,
    []
  );
  ...
  const handleOnAlertClick = (
    ...
    setReadAlerts((prevReadAlerts) =>
      prevReadAlerts.includes(id) ? prevReadAlerts : [...prevReadAlerts, id]
    );
    getGameStore().set({
      notificationData: {
        x: position.x,
  ...
  const hasNotification = alerts.data
    ? alerts.data.filter((alert) => !readAlerts.includes(alert.id)).length > 0
    : false;
      ...
      {!isOpen && (
        ...
        >
          <span className={cn(["relative"])}>
            ...
            <div
              className={cn([
                "absolute top-[-5px] right-[-5px] border h-2 w-2 rounded-full bg-brand-danger",
        ...
        <div className={cn(["h-full", "flex flex-col"])}>
          <div className={cn(["flex items-center"])}>
            <div className={cn(["grow py-xs"])}>
              <h2
                className={cn([
                  "text-[#FFC400] text-left text-base uppercase font-silkscreen",
                ])}
              >
                Event Logs
              </h2>
